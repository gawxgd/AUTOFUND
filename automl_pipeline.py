...
#     data_downloader = DataDownloader(data_version=data_version)
#     data_downloader.download_all_data()

#     data_loader = DataLoader(data_version=data_version, feature_set=feature_set)
#     data_manager = DataManager(data_version=data_version, feature_set=feature_set)
#     train_data = data_manager.load_train_data_for_base_models()
...
#     model_manager.train_base_models(train_data)
#     print("FINISHED TRAINING BASE MODELS")
#     model_manager.save_base_models()
#     print("FINISHED SAVING BASE MODELS")
    
#     data_for_creating_predictions = data_manager.load_data_for_creating_predictions_for_base_models()

#     predictions = model_manager.create_predictions_by_base_models(data_for_creating_predictions=data_for_creating_predictions)
#     data_manager.save_vanila_predictions_by_base_models(predictions= predictions)

#     print("FINISHED CREATING PREDICTIONS BY BASE MODELS")
#     validation_data = data_manager.load_vanila_predictions_data_by_base_models()


#     model_manager.find_neutralization_features_and_proportions_for_base_models(validation_data=validation_data,metric="sharpe", number_of_iterations=50, max_number_of_features_to_neutralize=120)
#     print("FINISHED FINDING NEUTRALIZATION FEATURES AND PROPORTIONS FOR BASE MODELS")
    
#     vanila_predictions_data = data_manager.load_vanila_predictions_data_by_base_models()
    
#     model_manager.create_neutralized_predictions_by_base_models_predictions(vanila_predictions_data=)
